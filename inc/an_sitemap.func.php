...
if(!function_exists("an_pagnav")) {
	...
	* @param string $modname - обязательный, имя файла модуля для первого параметра ф-ции sed_url (например: "plug")
	* @param string $sed_url_params - второй параметр для ф-ции sed_url (например: "e=games&cat=1")
	* @param boolean $per_page_sel - выводить возможность выбора количества элементов нв странице
	* $characters - параметр передающий номер страницы, будет добавлен к $sed_url_params
	*/
	function an_pagnav($current, $totalitems, $perpage = 10, $modname = '', $sed_url_params = '',  $characters = 'd', $per_page_sel = false){
		global $L, $cfg;
		...
		if ($current > 1){
			$prev = $current - 1;
			if ($prev < 1) { $prev = 1; }
			$prev = "<span class=\"pagenav_prev\"><a href=\"".sed_url($modname, $sed_url_params."&".$characters.'='.$prev)."\" title=\"".$L['plu_pagnav_prev_desc']."\">".$L['plu_pagnav_prev']."</a></span>";
		}else{
			$prev = '';
		}
		...
		if (($current) < $totalpages){
			$next = $current + 1;
			$next = "<span class=\"pagenav_next\"><a href=\"".sed_url($modname, $sed_url_params."&".$characters.'='.$next)."\" title=\"".$L['plu_pagnav_next_desc']."\">".$L['plu_pagnav_next']."</a></span>";
		}
		...
		if(($each_side * 2) >= $totalpages) {
			for ($k = 1; $k <= $totalpages; $k++){
				...
				if ($k==$currentpage){
					...
					$pages .= "<span class=\"pagenav_pages\"><a href=\"".sed_url($modname, $sed_url_params."&".$characters.'='.$k)."\">".($k)."</a></span>"; 
				}
			}
		}else{
			if ($currentpage > $each_side){
				$first = "<span class=\"pagenav_first\"><a href=\"".sed_url($modname, $sed_url_params."&".$characters.'=1')."\" title=\"".$L['plu_pagnav_first_desc']."\">".$L['plu_pagnav_first']."</a></span>";
			}
		 	if ($currentpage < $totalpages-$each_side){	
				$last = $totalpages;
				$last = "<span class=\"pagenav_last\"><a href=\"".sed_url($modname, $sed_url_params."&".$characters.'='.$last)."\" title=\"".$L['plu_pagnav_last_desc']."\">".$L['plu_pagnav_last']."</a></span>";
			}
		...
		if ($per_page_sel){
			...
			$per_page_url = $cfg['mainurl']."/".sed_url($modname, $sed_url_params."&".$characters.'=1');
